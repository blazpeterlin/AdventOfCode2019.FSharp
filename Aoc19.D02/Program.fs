...
let res1 = m0 |> parameterize 12L 2L |> runUntilHalt |> ics2codes |> getResult


let res2 = 
    Seq.allPairs [0L..99L] [0L..99L]
    |> Seq.map (fun (x,y) -> m0 |> parameterize x y |> runUntilHalt |> ics2codes |> fun m -> (x,y,getResult m))
...
let res2_alternative =
    seq {
        for x in [0L..99L] do
            for y in [0L..99L] do
                let z = m0 |> parameterize x y |> runUntilHalt |> ics2codes |> getResult
